# OpenID Connect Deployment Guide for auth.xcity.ai Integration

## Overview

This guide provides step-by-step instructions for configuring xct-chat to use auth.xcity.ai as the OpenID Connect authentication provider.

## Prerequisites

1. Access to auth.xcity.ai admin panel to register OAuth clients
2. Access to deployment environment (Railway, Docker, etc.)
3. Client credentials from auth.xcity.ai (CLIENT_ID and CLIENT_SECRET)

## Registration at auth.xcity.ai

Before configuring environment variables, you must register xct-chat as an OAuth client at auth.xcity.ai:

1. Log in to auth.xcity.ai admin panel
2. Create a new OAuth/OIDC client with the following settings:
   ...
   - **Post Logout Redirect URI**: `https://chat.xcity.ai/login`
...
```bash
# Client credentials from auth.xcity.ai
OPENID_CLIENT_ID=<your-client-id-from-auth.xcity.ai>
OPENID_CLIENT_SECRET=<your-client-secret-from-auth.xcity.ai>
...
Enable automatic redirect to auth.xcity.ai for unauthenticated users:

```bash
# Bypass login form and redirect directly to auth.xcity.ai
...
Enable logout synchronization with auth.xcity.ai:
...
For auth.xcity.ai-only authentication, disable local email/password login:
...
1. **Auto-redirect Test**
   - Visit `https://chat.xcity.ai` without being logged in
   - You should be automatically redirected to `https://auth.xcity.ai/authorize?...`

2. **Login Flow Test**
   - Complete login at auth.xcity.ai
   - Verify callback to `https://chat.xcity.ai/oauth/openid/callback`
   - Verify successful entry into chat interface

3. **Logout Sync Test**
   - Click logout in xct-chat
   - Verify you're logged out of auth.xcity.ai as well
   - Verify redirect to `https://chat.xcity.ai/login`
...
- Verify the callback URL registered at auth.xcity.ai exactly matches: `https://chat.xcity.ai/oauth/openid/callback`
- Check that `DOMAIN_CLIENT` is set to `https://chat.xcity.ai` (no trailing slash)
...
- Verify auth.xcity.ai allows `https://chat.xcity.ai` as an authorized origin
...
- auth.xcity.ai Documentation: Refer to auth.xcity.ai admin panel for OIDC provider details
